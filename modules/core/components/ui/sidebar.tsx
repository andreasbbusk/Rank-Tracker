...
import { Sheet, SheetContent } from "@/modules/core/components/ui/sheet";
    ...
    return (
      <>
        ...
        <div
          ...
          className={cn(
            "fixed inset-0 z-50 bg-black/50 transition-opacity duration-200 ease-linear",
            openMobile
...
const SidebarTrigger = ({
  ...
  const { toggleSidebar, open, platform } = useSidebar();
      ...
      <Tooltip>
        ...
        <TooltipContent className="border bg-background text-foreground">
          <p className="flex items-center gap-2">
            {open ? "Luk" : "Åbn"} sidebar
            <kbd className="pointer-events-none inline-flex h-5 select-none items-center gap-1 rounded border bg-muted px-1.5 font-mono text-[10px] font-medium text-muted-foreground opacity-100">
              <span
                className={cn(platform === "mac" ? "text-xs" : "text-[10px]")}
              >
                {platform === "mac" ? "⌘" : "Ctrl +"}
              </span>
              B
